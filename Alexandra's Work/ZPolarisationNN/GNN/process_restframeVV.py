...
Process for ZZ rest frame data 
...
graphsdir = "../graphdata/new_Input_Polarisations_ZZ__16th_November2024_ZZrest.h5" #directory to store graph data 
...
class MyOwnDataset(Dataset):
    ...
    def _preprocess(self, column, i):
        ...
        E_const = column[4] * np.cosh(column[2]) #energy of constituent  pT*cosh(eta)
        log_E_const = np.log(E_const)
        log_pT = np.log(column[4]) #log particle transverse momentum
        ...
        rel_pT = np.log(column[4] / p_ref) #log(pT/pT(lepton/jet))
        delta_eta = column[2] - eta_ref #difference in eta between constituent and lepton/jet
        delta_phi = column[3] - phi_ref #difference in phi 
        ...
        #add isLepton, charge
        nodes.extend([column[10], column[8]]) 
